...
Design Enxuto, Minimalista e Direto ao Ponto (Padrão Apple Fitness / Whoop / Hevy).
Foco na execução rápida na academia: GIF do movimento, ajuste fácil de cargas/reps e salvamento infalível.
"""
import flet as ft
import time
import threading
...
class WorkoutView:
    def __init__(self, page: ft.Page, on_view_exercise_guide=None, user_id: Optional[int] = None):
        ...
        self.timer_text = ft.Text("00:00", size=18, weight=ft.FontWeight.BOLD, color=SportColors.PRIMARY_NEON)
        self.timer_progress = ft.ProgressBar(value=0.0, color=SportColors.PRIMARY_NEON, bgcolor=SportColors.BG_SURFACE_ALT, height=3)
        ...
        self.exercise_cards_column = ft.Column(spacing=12)
    ...
    def build(self) -> ft.Control:
        ...
        # Cronômetro de Descanso Compacto
        timer_strip = SportStyles.card_container(
            content=ft.Column([
                ft.Row([
                    ft.Row([
                        ft.Icon(Icons.TIMER_OUTLINED, color=SportColors.PRIMARY_NEON, size=16),
                        ft.Text("DESCANSO:", size=11, weight=ft.FontWeight.BOLD, color=SportColors.TEXT_SECONDARY),
                        self.timer_text,
                    ], spacing=6),
                    ft.Row([
        ...
        # Coluna principal centralizada e responsiva Mobile First (Padrão Hevy / Apple Fitness)
        ...
        return ft.Container(
            ...
            ], spacing=0, padding=AppPadding.symmetric(horizontal=8, vertical=6)),
    ...
    def _render_current_routine_exercises(self):
        """Renderiza os cards completos de cada exercício com design limpo, GIF e séries."""
        ...
        for ex_idx, ex in enumerate(exercises):
            ...
            # 3. Tabela de Séries Mobile First (Padrão Hevy & Strong para Celulares)
            ...
            for s_num in range(1, target_sets + 1):
                ...
                check_btn = ft.Container(
                    content=ft.Icon(Icons.CHECK, size=16, color=SportColors.TEXT_MUTED),
                    bgcolor=SportColors.BG_SURFACE_ALT,
                    border=AppBorder.all(1, SportColors.BORDER_DEFAULT),
                    border_radius=6,
                    width=44,
    ...
    def _finish_workout_session(self):
        """
        Finaliza o treino de forma infalível:
        Se o usuário marcou as séries individualmente, salva as marcadas.
        Se NÃO clicou em cada série, coleta automaticamente todas as séries preenchidas na tela.
        NUNCA MAIS recusa ou descarta o treino do aluno!
        """
        uid = self.user_id or DBService.get_active_user_id()
        ...
        # Se não tiver séries marcadas manualmente, computa todas da tela automaticamente
        sets_to_save = list(self.completed_sets)
        if not sets_to_save:
            for ex in exercises:
                ex_name = ex.get("exercise_name", "Exercício")
                target_sets = ex.get("target_sets", 3)
                default_w = ex.get("target_weight", 0.0)
                inputs = self.active_inputs.get(ex_name, {})

                for s_num in range(1, target_sets + 1):
                    w_inp = inputs.get(f"s{s_num}_w")
                    r_inp = inputs.get(f"s{s_num}_r")
                    
                    try:
                        w_val = float(w_inp.value if w_inp and w_inp.value else default_w)
                    except Exception:
                        w_val = float(default_w or 0)
                        
                    try:
                        r_val = int(r_inp.value if r_inp and r_inp.value else 10)
                    except Exception:
                        r_val = 10
                    ...
                    ft.Container(
                        content=ft.Column([
                            ft.Row([
                                ...
                                ft.Text(f"{len(sets_to_save)} séries", size=11, weight=ft.FontWeight.BOLD, color=SportColors.PRIMARY_NEON)
                            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                            ft.Row([
                                ft.Text("Duração da Sessão:", size=11, color=SportColors.TEXT_SECONDARY),
                                ft.Text(f"{duration_min} min", size=11, weight=ft.FontWeight.BOLD, color=SportColors.TEXT_WHITE)
                            ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                            ft.Row([
                                ft.Text("Tonelagem Levantada:", size=11, color=SportColors.TEXT_SECONDARY),
